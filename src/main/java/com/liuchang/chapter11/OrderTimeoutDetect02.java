...
import com.liuchang.pojo.OrderEvent;
...
public class OrderTimeoutDetect02 {
    public static void main(String[] args) throws Exception {
        ...
        KeyedStream<OrderEvent, String> stream = env.fromElements(
                ...
                .assignTimestampsAndWatermarks(WatermarkStrategy.<OrderEvent>forMonotonousTimestamps().withTimestampAssigner((SerializableTimestampAssigner<OrderEvent>) (event, l) -> event.timestamp)).keyBy(order -> order.orderId); // 按照订单 ID 分组
        // 1. 定义 Pattern
        Pattern<OrderEvent, ?> pattern = Pattern.<OrderEvent>begin("create") // 首先是下单事件
