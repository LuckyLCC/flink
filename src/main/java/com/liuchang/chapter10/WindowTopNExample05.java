package com.liuchang.chapter10;
 ...
 * @Description: TODO
 * @Author: liuchang
 * @CreateTime: 2022-07-26  09:30
 */
public class WindowTopNExample05 {
...
// 将 timestamp 指定为事件时间，并命名为 ts
        ...
        String subQuery = "SELECT window_start, window_end, user, COUNT(url) as cnt " + "FROM TABLE ( " + "TUMBLE( TABLE EventTable, DESCRIPTOR(ts), INTERVAL '1' HOUR ))  GROUP BY window_start, window_end, user ";
        // 定义 Top N 的外层查询
        String topNQuery = "SELECT * " + "FROM (" +  "SELECT *, " +
        "ROW_NUMBER() OVER ( " + "PARTITION BY window_start, window_end " + "ORDER BY cnt desc " + ") AS row_num " + "FROM (" + subQuery + ")) " + "WHERE row_num <= 2";
        // 执行 SQL 得到结果表
